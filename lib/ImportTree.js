...
function gatherImport(importingFile, importedFile) {
  if (this.desc.has(importingFile) && this.desc.get(importingFile).includes(importedFile)) {
    this.warn(`Duplicated @import in file '${importingFile}': '${importedFile}'`)
  } else {
    if (!this.desc.has(importingFile)) {
      this.desc.set(importingFile, [])
    }
    if (!this.asc.has(importedFile)) {
      this.asc.set(importedFile, [])
    }
    this.desc.get(importingFile).push(importedFile)
    this.asc.get(importedFile).push(importingFile)
  }
}
...
module.exports = class ImportTree {
  ...
  removeImportedFile(file) {
    ...
    this.desc.forEach((importedFiles, importingFile) => {
      this.desc.set(importingFile, importedFiles.filter((importedFile) => importedFile !== file))
      if (0 === this.desc.get(importingFile).length) {
  ...
  removeImportingFile(file) {
    ...
    this.asc.forEach((importingFiles, importedFile) => {
      this.asc.set(importedFile, importingFiles.filter((importingFile) => importingFile !== file))
      if (0 === this.asc.get(importedFile).length) {
