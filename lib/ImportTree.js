...
module.exports = class ImportTree {
  ...
  findImportingFiles(importedFile) {
    let allImportingFiles = new Set()
    if (this.asc.has(importedFile)) {
      const importingFiles = this.asc.get(importedFile)
      allImportingFiles = new Set(importingFiles)
      importingFiles.forEach((importingFile) => {
        if (this.asc.has(importingFile)) {
          allImportingFiles = new Set([...allImportingFiles, ...this.findImportingFiles(importingFile)])
        }
      })
    }
